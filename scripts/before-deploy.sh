...
if [[ "$TRAVIS_BRANCH" == "$PRODUCTION_BRANCH" ]]; then
  ...
  aws s3 rm s3://$PRODUCTION_BUCKET --recursive --exclude "android/reference/*" --exclude "android/plugins/*"
  echo "Done"
fi

if [[ "$TRAVIS_BRANCH" == "$STAGING_BRANCH" ]]; then
  echo "Removing existing files on staging..."
  aws s3 rm s3://$STAGING_BUCKET --recursive --exclude "android/reference/*" --exclude "android/plugins/*"
  echo "Done"
fi
